...
def build_npmrc(data, key):
    template = Template("registry=${url} \n_auth=""${token}"" \nalways-auth = true \n")
    return template.substitute(url=data[key]["url"], token=data[key]["token"])

def build_scoped_npmrc(data, key):
    template = Template("${scope}:registry=registry=${url}\n//${domain}/:/authTokemn=${token}")
    domain = urlparse(data[key]["url"])
    return template.substitute(scope=data[key]["scope"], url=data[key]["url"], token=data[key]["token"], domain=domain.netloc)
